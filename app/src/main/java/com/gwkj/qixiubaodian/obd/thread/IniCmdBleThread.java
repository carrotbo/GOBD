...
public class IniCmdBleThread extends Thread
{
	private static IniCmdBleThread instanceInitThread = null;
	private OBDItem item;
	public static Handler rehandler;
	private boolean isBreak=false;

	public static IniCmdBleThread getInstnceInitThread(Handler handler)
	{
		rehandler=handler;
		if (instanceInitThread == null)
			instanceInitThread = new IniCmdBleThread();
		return instanceInitThread;
	}

	private IniCmdBleThread()
	{
...
//                            content_ly.addView(getRightTextView("发送到OBD："+data.getCmd()));
				...
				SendSocketService.sendBleMessage(data.getCmd(),null);
