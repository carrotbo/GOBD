...
public class BltManager {
    ...
    public void disConnectBlt(){
        ...
        try {
            if (getmBluetoothSocket() != null) {
                getmBluetoothSocket().getInputStream().close();
                getmBluetoothSocket().getOutputStream().close();
                getmBluetoothSocket().close();
                mBluetoothSocket=null;
            }
            if(BltAppliaction.bluetoothSocket!=null){
                BltAppliaction.bluetoothSocket.getInputStream().close();
                BltAppliaction.bluetoothSocket.getOutputStream().close();
                BltAppliaction.bluetoothSocket.close();
                BltAppliaction.bluetoothSocket=null;
            }
    ...
    private BroadcastReceiver searchDevices = new BroadcastReceiver() {
        ...
        public void onReceive(final Context context, Intent intent) {
            ...
            //状态改变时
                ...
                switch(blueState){
                    case BluetoothAdapter.STATE_TURNING_ON://打开开关自动扫描
                        Log.d("BlueToothTestActivity", "开关");
                        handler.sendEmptyMessageDelayed(1,1000);
                        break;
                    case BluetoothAdapter.STATE_ON:
                        Log.d("BlueToothTestActivity", "开始扫描");
                        //开始扫描
                        break;
                    case BluetoothAdapter.STATE_TURNING_OFF:
                        Log.d("BlueToothTestActivity", "关");
                        break;
                    case BluetoothAdapter.STATE_OFF:
                        Log.d("BlueToothTestActivity", "扫描关");
                        break;
    ...
    private void connect(BluetoothDevice btDev, Handler handler) {
        try {
            ...
            Toast.makeText(context, "设备" + btDev.getName() + "已经接入", Toast.LENGTH_LONG).show();
            if (handler == null) {
                return;
            }
            //结果回调
            ...
            if (handler == null) {
                Toast.makeText(context, "链接失败请重连", Toast.LENGTH_LONG).show();
                return;
            }
            e.printStackTrace();
            Message message = new Message();
    ...
    public void connectChange(BluetoothDevice btDev, Handler handler) {
        try {
            //通过和服务器协商的uuid来进行连接
            mBluetoothSocket = btDev.createRfcommSocketToServiceRecord(BltContant.SPP_UUID);
            if (mBluetoothSocket != null)
