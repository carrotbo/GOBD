...
public class TableCmdThread extends Thread
{
	private static TableCmdThread instanceInitThread = null;
	private static List<OBDPid.DataBean> mlist;
	public static Handler rehandler;
	private boolean isBreak=false;

	public static TableCmdThread getInstnceInitThread(Handler handler, List<OBDPid.DataBean> list)
	{
		mlist=list;
		rehandler=handler;
		if (instanceInitThread == null)
			instanceInitThread = new TableCmdThread();
		return instanceInitThread;
	}

	private TableCmdThread()
	{
	}
	...
	public void run()
	{
		...
		for (int i = 0; i < length; i++) {
			...
			synchronized (Common.sendLock2) {
				...
				rehandler.sendEmptyMessageDelayed(6,1000);
				try
				{
